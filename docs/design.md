...
/handoff [--model provider/model-id[:thinking]] [scope]
  ...
  │
  ├─ GATE A     BLUF + definition of done + twelve-line prompt preview + model + rationale
...
Rendered with `ctx.ui.custom` so the BLUF, definition of done, prompt preview, model line, and rationale are visible together. The BLUF and definition block sit above the fixed twelve-line preview; older drafts without metadata show an unobtrusive BLUF fallback. A leftovers draft is labelled with the accepted slug and keeps its rationale above the metadata; without a definition of done, Cancel moves ahead of Run. The model line names the **Change model** option and marks a `--model` choice. Options:
...
While running, a widget keeps the BLUF and a width-truncated, bounded definition-of-done header above elapsed time, turns, tokens, cost, and the last few tool calls. It shows two completion bullets in a 24-row terminal and up to all five on taller terminals, while reserving room for metrics, recent activity, and the abort hint. The reviewing session's agent is not invoked during this phase; the command handler awaits the child process directly, so the user sees the widget and can press Escape in it to stop the worker. The overlay owns the `AbortController` for the run because there is no external signal to borrow: `ExtensionCommandContext` has no abort member, and `ExtensionContext.signal` is undefined when the agent is not streaming, which is exactly the case while a command handler awaits a child process.
