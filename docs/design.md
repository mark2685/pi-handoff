...
/handoff [--model provider/model-id[:thinking]] [scope]
  ...
  │              [View full report | View partial output] [View full diffstat] [Accept and hand off leftovers]
  │
...
While running, a widget keeps the goal line and a width-truncated, bounded definition-of-done header above a live worker-status row, elapsed time, turns, tokens, cost, and the last few tool calls. The status row names Pi's current phase (thinking, writing, preparing a tool, finalizing), lists active tools from `tool_execution_start`/`tool_execution_end`, and states the age of the last worker event. Its pulse only says the overlay is live: a silent child is explicitly shown as awaiting its first event or with an old last-event age, never as making unobserved progress. A no-progress watchdog resets on each of those meaningful lifecycle events; after the configurable `DEFAULT_NO_PROGRESS_THRESHOLD_MS` default of ten minutes without one, it emits a stalled activity through that same progress channel. The overlay then explicitly offers the user the choice to keep waiting or press Escape to stop the worker. The watchdog never calls the abort path or kills the child: long model requests and slow tools are normal, so Escape remains the only route that ends a run. The declared 24-row terminal minimum applies to this widget and the read-only viewer: the widget shows two completion bullets there and up to all five on taller terminals, while reserving room for metrics, recent activity, and the abort hint. Gate A and Gate B use the same terminal-row source for their separate preview budgets, but their larger fixed summaries need roughly 30 and 40 rows respectively for the common states. The reviewing session's agent is not invoked during this phase; the command handler awaits the child process directly, so the user sees the widget and can press Escape in it to stop the worker. The overlay owns the `AbortController` for the run because there is no external signal to borrow: `ExtensionCommandContext` has no abort member, and `ExtensionContext.signal` is undefined when the agent is not streaming, which is exactly the case while a command handler awaits a child process.
...
Rendered with `ctx.ui.custom`, showing the report, `git diff --stat` against the checkpoint, usage, and iteration count. Its report, diffstat, review, and crash-tail previews share the rows left after fixed metadata, menu, and chrome: each keeps a three-rendered-row floor and, when text is hidden, a signpost naming the viewer that reveals it, with worker report first, then reviewer findings, diffstat, and crash tails. A common completed Gate B fits at about 40 rows; the 24-row minimum cannot fit its largest states without giving up non-preview content. Options:
...
- **Send feedback to worker** — `ctx.ui.editor` is prefilled with the captured reviewer text when there is one, with a trailing `Verdict:` line removed using the same tolerated forms described above. With no review and an **interrupted** iteration there is nothing to prefill and never will be, because Review here is refused for a run that produced no report, so the editor instead opens on an editable resume draft built by a pure domain function from the interruption note: it says the previous iteration did not finish and was not reviewed, quotes the note verbatim as the only account of what ended the run, states that the working tree still holds that run's changes and that its unfinished output is not a report, and asks the worker to read the current state, finish the remaining work without starting over or reverting, and write the real final report. The draft promotes nothing: it is feedback text a person edits or deletes, and `partialReport` remains pre-crash output. With no review and a **completed** iteration the editor stays blank, since a report the user has not reacted to yet gives nothing to draft from; the hint moves into the editor title (`no review captured for iteration N; Review here would pre-fill this`), because `ctx.ui.editor` takes only a title and a prefill and a bare empty buffer reads as a fault. A verdict-only review gets its own title, since claiming no review was captured would be false. One pure function (`buildFeedbackEditorRequest`) resolves all four cases into a title and a prefill, so the two cannot disagree about which case it is. The user still confirms the text by editing or submitting it. Normalization happens before the emptiness check, so a verdict-only submission is refused without starting a worker iteration. Enter submits and the editor trims, so an untyped submission arrives as an empty string: the two refusals are worded apart — an empty buffer is reported as empty and names how to type multi-line feedback, while only text that was nothing but a `Verdict:` line is reported as verdict-only. Both return to Gate B without starting an iteration rather than reopening the editor, keeping one exit from the branch and leaving Escape as the way out. The old shared verdict-only wording was the dead end that mattered: an empty submit was described as text the user never wrote, and users who hit it repeatedly accepted a handoff instead. The empty-buffer refusal names the option that would draft the text for them, and names it as the menu renders it — **Review here** with no review, **Review again** after a verdict-only one, and nothing at all for an interrupted run, whose review option the gate does not offer.

  A captured review whose text is blank — what a review turn that failed on a provider error, or ended on a message with no text, stores — is dropped where Gate B's view is built, by one predicate (`hasReviewEvidence`) the gate, the editor, and the prompt all read. Presence of a review is what relabels the menu's actions and renders the reviewer block, so reinterpreting an empty one further downstream produced a single render that offered **Send review to worker** above an empty findings block while the editor it opened said no review was captured. A review that is only a `Verdict:` line is kept, because the verdict is real evidence and it orders the gate's actions; it has no findings to prefill, which is a different thing and is worded differently.

  The extension normalizes that feedback the same way before appending a section to the prompt file. What that section may claim depends on where the text came from, because it tells the worker who inspected its tree: a `review` source keeps `## Review feedback (iteration N)` and the preamble that says a reviewer inspected the tree and that only those findings are in scope; a `user` source (a finished iteration nobody reviewed) and a `resume` source (an interrupted one) use `## Instructions from the user (iteration N)` and say the instructions are the user's, with the resume wording adding that the previous iteration did not finish and asking the worker to finish the handoff's remaining work. Every source keeps the iteration number, the seven-character original checkpoint, and the rule that prior changes must not be restarted or reverted unless the instructions say so. The split exists because one fixed preamble put contradictory claims in one section: "address only the reviewer findings; do not expand scope" directly above a resume draft stating no review was captured and asking for the rest of the task, which a worker can read narrowly and leave the handoff unfinished. It then returns to 5.4. Refused when `iteration >= maxIterations`. The restart reuses the checkpoint taken before iteration 1 rather than taking a new one, so Discard still undoes every iteration and the diffstat stays cumulative across the loop. It drops `external`, but carries a true `autoReview` latch; a completed retry therefore starts the review turn immediately, while an interrupted retry renders Gate B as usual.
...
- **View full report** / **View partial output** / **View full diffstat** — open the whole completed report, a crashed worker's pre-crash text, or the whole diffstat in the read-only viewer of §5.8, then re-render Gate B unchanged. Pre-crash text is never called a report: its viewer is labelled **View partial output** and titled as not a report. A viewer is omitted when its height-budgeted preview already showed all of its text, because opening a no-op reader would imply hidden content. The diffstat view also carries the stderr tail when a crash left one, so a shortened stderr preview signs the actual **View full diffstat** route rather than inventing a separate crash viewer.
...
- A worker with no meaningful lifecycle event for the configured no-progress threshold shows the stalled prompt while its child process remains alive; an intervening event resets the timer, and the watchdog never kills the child.
